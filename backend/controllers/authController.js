...
exports.googleCallback = async (req, res) => {
  try {
    ...
    res.redirect(`${frontendUrl}/pages/login.html?token=${token}&name=${encodeURIComponent(user.name)}`);
  } catch (error) {
    const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:8000';
    res.redirect(`${frontendUrl}/pages/login.html?error=authentication_failed`);
  }
};
